...
void Util::toString(char string[], int size, bool value){
	...
	//qui sarebbe meglio mettere un controllo sulle dimensioni di string, ossia controllare
	//se ci sta false (che è la stringa più lunga) (if(size>strlen("false"))) altrimenti
	//la funzione strcat potrebbe sovrascrivere locazioni di memoria gia utilizzate.  
	//un altro modo per deviare il problema è usare strncat.. vedi tu! 
	if (value == true){
		strcat(string, "True");
	}else{
		strcat(string, "False");
