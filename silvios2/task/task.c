...
void add_task(void *eip, unsigned char *nome)
{
	...
	task[num_task].esp = (unsigned int) malloc(4095);
	...
	task[num_task].ecx = 0;
	task[num_task].edx = 0;
	task[num_task].ebp = (unsigned int) malloc(4095);
	task_desk[num_task] =
...
void get_curtask_status()
{
	printf("\nPID = %d, EAX = 0x%x, EBX = 0x%x, ECX = 0x%x, EDX = 0x%x",
	       cur_task, task[num_task].eax, task[num_task].ebx,
	       task[num_task].ecx, task[num_task].edx);
	printf("\nCS = 0x%x, DS = 0x%x, SS = 0x%x, ES = 0x%x, FS = 0x%x, GS = 0x%x", task[num_task].cs, task[num_task].ds, task[num_task].ss, task[num_task].es, task[num_task].fs, task[num_task].gs);
	printf("\nESP = 0x%x, EBP = 0x%x,  EIP = 0x%x, CR3 = 0x%x, EFLAG = 0x%x", task[num_task].esp, task[num_task].ebp, task[num_task].cr3, task[num_task].eflags);
	return;
}
