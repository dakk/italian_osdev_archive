...
void*  prendi_pagina_fisica(){
	...
	if(pagina_libera > num_pages){
	        debug << "finito lo spazio a disposizione" << endl;        
		return NULL;
	}
	debug << "trovata pagina libera numero: " << pagina_libera << endl;
...
bool rilascia_pagina(void *addr){
        ...
        if(indirizzo % DIM_PAGE){
	        debug << "pagina non valida" << endl;
	        return false;
	}
	debug << "libero " << (indirizzo/DIM_PAGE) << endl;
