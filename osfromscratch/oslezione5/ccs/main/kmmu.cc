...
void init_kmmu() {
   page_table_entry*  current_table;
    ...
    kernel_dir = (page_dir_entry*)prendi_spazio_fisico((void*)0, (void*)(sizeof(page_dir_entry)*1024));
    
    kout << "la directory del kernel inizia a " << (dword)kernel_dir << endl;
    
    //prendo la prima tabella del kernel, quella che mapperà i primi 4 mega virtuali
    current_table = (page_table_entry* ) prendi_spazio_fisico((void*)4096, (void*)(4096 + sizeof(page_table_entry)*1024));
    
    kout << "la prima tabella del kernel inizia a " << (dword)current_table << endl;
    
    //imposta la prima entry nella tabella del kernel
    kernel_dir[0] =    crea_page_dir(current_table, SCRITTURA_LETTURA | PRESENTE);
  ...
  dword addr = (dword)prendi_spazio_fisico((void*)0, (void*)(1<<20));
   for (int i = 0; i < 256; i++){
	current_table[i] =  crea_page_table((page_base *) addr,  SCRITTURA_LETTURA | PRESENTE);
	addr += 4096;
  }
