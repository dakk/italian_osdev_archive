 ...
 ***************************************************************************
 * $Id: signals.c,v 1.3 2000/10/23 23:33:44 iurlano Exp $
 ***************************************************************************
 *
 * $Log: signals.c,v $
 ...
 *
 *
 * Revision 1.1  1999/12/25 11:13:39  iurlano
 * *** empty log message ***
 *
...
void set_signal_handler(unsigned long int sigg,Signal_Handler kk) {
  __asm__ ("int $0x40"::"a" (SYS_SIGNAL_HANDLER),"c" (sigg),"d" (kk));
}

int Signal(PID Processo,dword segnale) {
  int ret;
  __asm__ ("int $0x40":"=a" (ret):"a" (SYS_SIGNAL),"c" (Processo),"d" (segnale));
  return ret;
}
