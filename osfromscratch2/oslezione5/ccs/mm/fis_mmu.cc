...
void init_spazio_fisico(){
	...
	kout << "Rilevati " << num_pages << "byte ------->      " << (num_pages/1024) << " KB   ------>  "<< (num_pages/(1024*1024)) << " MB " << endl;
...
void*  prendi_pagina_fisica(){
	...
	if(pagina_libera > num_pages){
	        debug << "finito lo spazio fisico a disposizione" << endl;        
		return NULL;
	}
	debug << "trovata pagina fisica libera numero: " << pagina_libera << endl;
...
bool rilascia_pagina(void *addr){
        ...
        if(indirizzo % DIM_PAGE){
	        debug << "pagina  fisica non valida" << endl;
	        return false;
	}
	debug << "libero pagina fisica: " << (indirizzo/DIM_PAGE) << endl;
...
void* prendi_spazio_fisico(void *iinizio, void *ifine){
	...
	for(size_t i =  pinizio;i < pfine; i++ ){
		...
		debug << "setto la pagina fisica " << i << endl;
	}
...
void  libera_spazio_fisico(void *iinizio, void *ifine){
	...
	for(size_t i =  pinizio;i < pfine; i++ ){
		...
		debug << "libero la pagina fisica  " << i << endl;
	}
}
