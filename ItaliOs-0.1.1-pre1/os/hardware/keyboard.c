...
void keyb_handler(void)
{
	...
	if (!code224) {
		...
		if (!(led_status & LED_NUM_LOCK))
			if (((key >= 71) && (key <= 73))
			    || ((key >= 75) && (key <= 77)) || ((key >= 79)
								&& (key <=
								    83)))
				ch = 0;
	} else {
		//tabella speciale
		//printf("\ngestione tabella speciale");
		...
		if (key == 73)	/*pagina su */
			...
			ch = map[key][1];	//l'invioi del trastierino numerico
		else if (control_keys & CK_ALT) {	//se è l'alt gr
			ch = map[key][4];
			code224 = !code224;
			//      printf("\ntasti speciali con alt gr e gestione tasto %d", key);
		}
		//      else
		//              printf("\nnessuna condizione verifcata");
	}
	if (key <= 103) {

		if (ch) {
			//      printf("\nmetto nel buffer %d", ch);
