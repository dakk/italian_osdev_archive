...
BOOL read(int block, void *buffer, unsigned int size)
{
	...
	if (size <= 512) {
		...
		psize = ((size / 512) * 512) + 512;
	}
	mbuffer = (unsigned char *) malloc(psize);
	temp = read_block(block, (BYTE *) & mbuffer, psize / 512);
	memcpy(buffer, mbuffer, size);
	return temp;
}


BOOL write(int block, void *buffer, unsigned int size)
{
	unsigned char *mbuffer;
/*    BOOL temp;  mk*/
	unsigned int psize;
	...
	mbuffer = (unsigned char *) malloc(psize);
	memset(mbuffer, 0, psize);
	memcpy(mbuffer, (BYTE *) buffer, size);
	return write_block(block, (BYTE *) & mbuffer, psize / 512);
}
...
// The FDC can read multiple sides at once but not multiple tracks
	...
	// debug("\nread_block: Sto leggendo il blocco %d", block);
...
{
//    debug("\nwrite_block: Sto scrivendo il blocco %d", block);
...
BOOL fdc_rw(int block, BYTE * blockbuff, BOOL read, unsigned long nosectors)
{
	...
	for (tries = 0; tries < 3; tries++) {
		...
		if (inportb(FDC_DIR) & 0x80) {
			...
			recalibrate();
			motoroff();
			kprint("FDC: Hai cambiato il disko, rimetti quello buono, e riprova\n");
			delay(500);
		...
		if (!seek(track)) {
			motoroff();
			kprint("FDC: errore nel trovare la traccia %d\n",
			       track);
			getch();
			return FALSE;
		}
		...
		if (!waitfdc(TRUE)) {
			kprint("Timed out, riprovare con un reset()\n");
			reset();
			return fdc_rw(block, blockbuff, read, nosectors);
		}
