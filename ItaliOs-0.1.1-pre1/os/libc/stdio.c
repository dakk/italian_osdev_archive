...
#define TUTTI 0
unsigned int uso_printf = TUTTI;	// se è uguale a zero hanno tutti dirizzo all'uso del video altrimenti ha il pid del processo che lha presa
...
void prendi_schermo(void)
{
	if (uso_printf == TUTTI)
		uso_printf = prendi_pid_task_corrente();
	else {
		while (uso_printf != TUTTI);	//aspetto che diventa uguale a TUTTI
		uso_printf = prendi_pid_task_corrente();	//ne prendo imamdiatamente il controllo ahahhahahhaha
	}
	return;
}


void rilascia_schermo(void)
{
	uso_printf = TUTTI;
	return;
}
 ...
 */
		...
		//      printf("\nrilevato un bel backspace");
		//      beep();
		x--;
		if (x < 0) {
			printf("\nx vale %d e abs_y vale %d", x, abs_y);
			x = 79;
			abs_y--;
		}
		dest = (char *) (0xb8000 + 2 * x + 160 * abs_y);	/* Video memory coords */
...
void kprint(char *format, ...)
{
	...
	if (uso_printf)
		if (prendi_pid_task_corrente() != uso_printf)
			return;
	...
	while ((curchar = *format++)) {	/* Loop through the string        */
		...
		case '%':	//elaboro le variabili
			...
			case 's':	//stampo una stringa
				...
				break;
			case 'c':	//stampo una stringa
				putc(attribute, va_arg(pt, unsigned char));
				break;
			}
			break;
